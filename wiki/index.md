...
- [튜토리얼 조작 흐름·UX 플레이테스트 보고서](playtest-tutorial-ux-flow-2026-08-09.md) — 접시 우선 흐름, 대화·화살표 레이어와 선택 방지의 실제 완주 결과.
...
- [Unity 모바일 포트 계획](plan-unity-mobile-port.md) — Phase 0–3.1 구현 상태, 웹 시각 에셋·번들 글꼴 이식과 남은 모바일 실기기 검증.
- [웹판 모바일 UI·버그 보정](plan-web-mobile-audit-2026-09-07.md) — 가로 고정 표시, 회전 좌표 보정, 작은 화면 UI와 입력·재도전 버그 검증.
